...
- `com.mindflow.mcp` 包（4 个子包，约 15 个类、~1500 行代码）
...
`com.mindflow.hitl.ApprovalPolicy.requiresApproval` 改为：
...
`com.mindflow.tool.ToolRegistry.AUDIT_TOOLS` 扩展为「危险工具集 + 任何 `mcp__` 前缀工具」。审计字段保持不变；`tool` 字段就是 `mcp__filesystem__read_file`，过滤时按前缀。
