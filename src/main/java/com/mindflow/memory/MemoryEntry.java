...
public class MemoryEntry {
    private final String id; // 唯一标识符（如 "user-abc123"）
    private final String content;// 记忆内容（对话文本、事实描述等）
    private final MemoryType type; // 记忆类型（枚举）
    private final Instant timestamp;// 创建时间戳
    private final Map<String, String> metadata; // 元数据（来源、工具名等）
    private final int tokenCount; // Token 数量
     ...
     */
    // Token 估算算法（中文约 1.5 字/token，英文约 4 字符/token）
