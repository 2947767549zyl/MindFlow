...
这份文档是 `mindflow` 仓库给各类 Agent / 新线程使用的首读入口。
...
- 结论：如果你看到运行界面是 `v14.0.0`，但 Jar 名仍是 `mindflow-1.0-SNAPSHOT.jar`，这是当前仓库的真实状态，不是你看错
...
# MINDFLOW_LOG_DIR=/Users/yourname/.mindflow/logs
# MINDFLOW_LOG_MAX_HISTORY=7
# MINDFLOW_LOG_MAX_FILE_SIZE=10MB
# MINDFLOW_LOG_TOTAL_SIZE_CAP=100MB
```

长期记忆默认持久化位置：

1. `~/.mindflow/memory/long_term_memory.json`
2. 如果传入 `-Dpaicli.memory.dir=/path/to/dir`，则优先使用该目录

代码索引（RAG）默认持久化位置：

1. `~/.mindflow/rag/codebase.db`
2. 如果传入 `-Dpaicli.rag.dir=/path/to/dir`，则优先使用该目录

操作审计日志默认持久化位置（以代码实际行为为准）：

1. 系统属性：`-Dpaicli.audit.dir=/path/to/dir`
2. 环境变量：`MINDFLOW_AUDIT_DIR`
3. 默认值：`~/.mindflow/audit/audit-YYYY-MM-DD.jsonl`（按天分文件，JSONL 格式）
...
1. 系统属性：`mindflow.log.dir`、`mindflow.log.level`、`mindflow.log.maxHistory`、`mindflow.log.maxFileSize`、`paicli.log.totalSizeCap`
2. 环境变量或 `.env`：`MINDFLOW_LOG_DIR`、`MINDFLOW_LOG_LEVEL`、`MINDFLOW_LOG_MAX_HISTORY`、`MINDFLOW_LOG_MAX_FILE_SIZE`、`PAICLI_LOG_TOTAL_SIZE_CAP`
3. 默认值：`~/.mindflow/logs` / `INFO` / `7` / `10MB` / `100MB`

ReAct / SubAgent 预算配置读取顺序（以代码实际行为为准）：

1. 系统属性：`mindflow.react.token.budget`、`mindflow.react.stagnation.window`、`mindflow.react.hard.max.iterations`
...
1. 系统属性：`mindflow.llm.connect.timeout.seconds`、`mindflow.llm.read.timeout.seconds`、`mindflow.llm.write.timeout.seconds`、`paicli.llm.call.timeout.seconds`
...
Web 抓取（`web_fetch`）安全策略（实现位于 `src/main/java/com/mindflow/web/NetworkPolicy.java`）：
...
1. 用户级：`~/.mindflow/mcp.json`
2. 项目级：`.mindflow/mcp.json`
...
第 13 期后，`~/.mindflow/mcp.json` 不存在时启动会自动创建默认 chrome-devtools 配置：
...
- 默认模式
- 主入口在 `src/main/java/com/mindflow/agent/Agent.java`
- 维护对话历史
- 退出条件由 LLM 自决：只要它不再返回 `tool_calls`、直接给出 `content`，循环就结束
- `AgentBudget`（`src/main/java/com/mindflow/agent/AgentBudget.java`）只承担保险阀职责，三种兜底任一命中即收尾：
...
主模块在 `src/main/java/com/mindflow/context/`：
...
- `AgentBudget.fromLlmClient()` 默认使用 `80% * maxContextWindow`，但 `mindflow.react.token.budget` 仍可覆盖
...
- 主入口在 `src/main/java/com/mindflow/agent/PlanExecuteAgent.java`
...
- 主模块在 `src/main/java/com/mindflow/memory/`
- 默认包含：短期记忆、长期记忆、摘要压缩、事实提取、Token 预算、相关记忆检索
- 注入到 system prompt 的”相关记忆”应只来自长期记忆；当前轮用户输入和短期对话已经在消息历史里，不应再被当成”历史记忆”重复注入
- 上下文有**两道压缩**，不要混淆：
  1. `ContextCompressor` 压缩 `MemoryManager.shortTermMemory`（mindflow 自己的短期记忆条目），由 `addUserMessage` / `addAssistantMessage` / `addToolResult` 内嵌触发，阈值看 `ConversationMemory.tokenCount` vs `ContextProfile.compressionTriggerRatio`
...
- 主模块在 `src/main/java/com/mindflow/rag/`
...
- 主入口在 `src/main/java/com/mindflow/agent/AgentOrchestrator.java`
...
- 主模块在 `src/main/java/com/mindflow/hitl/`
...
HITL 是"用户在场时确认"，本子段是 HITL 之外的辅助层，不是沙箱、不提供进程隔离。主模块在 `src/main/java/com/mindflow/policy/`：
...
- `AuditLog`：危险工具（`write_file` / `execute_command` / `create_project`）调用一律落一行 JSONL 到 `~/.mindflow/audit/audit-YYYY-MM-DD.jsonl`，字段：`timestamp / tool / args / outcome (allow|deny|error) / reason / approver (hitl|policy|none) / durationMs`。审计写入失败仅 stderr 提示，不影响主流程
...
- 主入口在 `src/main/java/com/mindflow/tool/ToolRegistry.java`
...
- 主模块在 `src/main/java/com/mindflow/web/`
...
- 主模块在 `src/main/java/com/mindflow/mcp/`
- 支持 stdio 子进程 server 与 Streamable HTTP 远程 server
- `McpConfigLoader` 读取 `~/.mindflow/mcp.json` 和 `.mindflow/mcp.json`，项目级按 server 名覆盖用户级
...
- 当前不实现 `/mcp add` / `/mcp remove`，配置编辑通过文件 + 重启 mindflow
...
- 默认配置文件：`~/.mindflow/mcp.json` 不存在时由 `Main.ensureDefaultMcpConfig()` 自动创建；已有文件缺少 `chrome-devtools` 时只提示，不覆盖用户配置
- 工具注册名：`mcp__chrome-devtools__navigate_page`、`mcp__chrome-devtools__take_snapshot`、`mcp__chrome-devtools__click` 等
- `McpClient.initialize()` 默认超时 60 秒，可用系统属性 `mindflow.mcp.initialize.timeout.seconds` 或环境变量 `MINDFLOW_MCP_INITIALIZE_TIMEOUT_SECONDS` 覆盖
...
- shared 模式下 `close_page` 只允许关闭 mindflow 自己创建的 tab；无法证明是 mindflow 创建的 tab 时策略层拒绝，HITL 无法批准绕过
- 敏感页面策略在 `src/main/java/com/mindflow/browser/SensitivePagePolicy.java`，内置 GitHub settings、支付、云控制台、飞书/Lark admin 等规则；用户可用 `~/.mindflow/sensitive_patterns.txt` 追加 glob 规则
...
- 主模块：`src/main/java/com/mindflow/skill/`
- 三层加载位置（按优先级，后者整体覆盖同名 skill）：
  1. jar 内置 `resources/skills/<name>/`
  2. 用户级 `~/.mindflow/skills/<name>/SKILL.md`
  3. 项目级 `<project>/.mindflow/skills/<name>/SKILL.md`
- 启动期 `SkillBuiltinExtractor` 把 jar 内置 skill 解压到 `~/.mindflow/skills-cache/<name>/`，按 `.version` 文件控制重建（避免每次启动 IO）
- 启用状态持久化：`~/.mindflow/skills.json` 的 `disabled` 列表，默认全启用
...
- 内置工具 `load_skill(name)`：LLM 在 system prompt 索引段里看到匹配 description 时主动调用，mindflow 把 SKILL.md 正文（去 frontmatter，5KB 截断）写入 `SkillContextBuffer`
...
- 站点经验三段式标准：`platform-features / valid-patterns / known-pitfalls`，frontmatter `domain / aliases / updated`；自我增强机制由 SKILL.md 里的提示驱动（"操作中发现新模式时主动写到 `~/.mindflow/skills/web-access/references/site-patterns/<domain>.md`"）
...
### `src/main/java/com/mindflow/cli/Main.java`
...
### `src/main/java/com/mindflow/agent/Agent.java`

- ReAct 主循环
- 对话历史维护
- 工具调用执行与结果回灌

### `src/main/java/com/mindflow/agent/PlanExecuteAgent.java`
...
### `src/main/java/com/mindflow/agent/AgentOrchestrator.java`
...
### `src/main/java/com/mindflow/agent/SubAgent.java`
...
### `src/main/java/com/mindflow/agent/AgentRole.java`

- Agent 角色枚举：PLANNER、WORKER、REVIEWER
- 每个角色有显示名和描述

### `src/main/java/com/mindflow/agent/AgentMessage.java`

- Agent 间通信消息类型
- 五种消息类型：TASK、RESULT、FEEDBACK、APPROVAL、REJECTION

### `src/main/java/com/mindflow/plan/Planner.java`
...
### `src/main/java/com/mindflow/plan/ExecutionPlan.java`

- DAG 拓扑排序
- 可执行任务判定
- 进度、状态、可视化与摘要

### `src/main/java/com/mindflow/tool/ToolRegistry.java`
...
### `src/main/java/com/mindflow/mcp/`
...
### `src/main/java/com/mindflow/llm/GLMClient.java`
...
- `src/main/java/com/mindflow/web/` 下相关文件
...
- `src/main/java/com/mindflow/policy/` 下相关文件
...
- `.env.example`：新增的环境变量示例（如 `MINDFLOW_AUDIT_DIR`）
...
- `src/main/java/com/mindflow/mcp/` 下相关文件
