...
//这个类负责处理一种特定的长期记忆场景——浏览器登录态复用。
// Agent 遇到需要登录的页面时，会调用 browser_connect 复用用户已登录的 Chrome。但如果用户每次都让它做同样的事情，
// Agent 应该记住用户的偏好，下次不用再说。
//    这个类的职责就是：用户说了一句涉及浏览器登录态的话，它判断是否要生成一条长期记忆。
